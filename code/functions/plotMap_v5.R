...
if(length(ship)<2){
  ...
  colors_enc<-unique(vs$SpColor)
  shapes_enc<- unique(vs$SpSymbol)
  ...
  base_map <- ggplot() + 
    ...
    ggspatial::layer_spatial(effort, alpha=0.1, size=0.1, aes(color=colors_lines[2]))+
    ggspatial::layer_spatial(tmp, alpha=0.1, size=0.1, aes(color=colors_lines[1]))+
