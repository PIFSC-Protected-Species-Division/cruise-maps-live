...
To re-run a day (perhaps the DAS was updated/corrected), just delete that DAS file from the `dasList` data.frame (which keeps track of what has been run). Use the following as a guide:
...
To re-run all days, just delete the `dasList.Rda` file from the 'outputs' folder. 
